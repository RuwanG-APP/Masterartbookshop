  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-md border border-gray-100">
          ...
          {cart.length > 0 ? (
            cart.map((item, index) => (
              <div key={index} className="flex justify-between items-center border-b border-blue-200 py-2 last:border-0">
                ...
                <button onClick={() => removeFromCart(index)} className="text-red-500 font-black hover:bg-red-100 px-2 py-1 rounded-lg">X</button>
              </div>
            ))
          ) : (
        ...
        <button onClick={() => window.location.href = "/"} className="w-full mb-4 py-2 text-[10px] font-bold text-blue-500 border-2 border-dashed border-blue-200 rounded-xl hover:bg-blue-50 uppercase">
          + තවත් පොත් එක් කරන්න
        </button>
        ...
        <div className="space-y-3">
          ...
          {paymentMethod === "Bank Transfer" && (
            <div className="p-3 border rounded-xl bg-yellow-50 text-[11px] text-gray-700">
              <p className="font-bold mb-1 uppercase text-yellow-800">බැංකු විස්තර:</p>
              <p className="font-bold text-blue-600">BOC - 123456789 (Narammala)</p>
